...
//            2018-10-20       ab  preliminary distributed control defines
...
//            2022-08-14       ab  added BIDIB_ACCESSORY_PARA_USES_STRINGS and FEATURE_STRING_NAMESPACES_AVAILABLE
//            2022-12-08       ab  added defines for MSG_STRING namespaces
//            2023-08-07       ab,ak,at,rk distributed control defines for rev1.29
//                                 added MSG_GUEST_REQ_*, MSG_GUEST_RESP_* for send wishes and subscriptions
//                                 added defines for target modes
//                                 added t_bidib_msg_type_filter, t_bidib_guest_subscription
//            2024-05-28       rk  added BIDIB_LINK_DESCRIPTOR_ROLE for netBiDiB/webBiDiB
//            2024-05-27       kw  changed FEATURE_BM_RCPLUS_AVAILABLE to FEATURE_BM_EXT_AVAILABLE (with Bit 2 for DCCA)
...
//===============================================================================
//
// content:   0. BiDiB general
//            1. Downstream Message Types
//            2. Upstream Message Types
//            3. Structure of Message Parameters
//            4. Feature Codes
//            5. Error Codes
//            6. Transport layers: Serial Link, BiDiBus, netBiDiB
//            7. Defines for System Messages and FW Update
//            8. Defines for Booster and Command Station Handling
//            9. Defines for IO-Control and Macro
...
//    (2021-04-23: added work around for BORLANDC)
//===============================================================================
        ...
        struct
          {
            ...
            uint8_t class_ui: 1; // guest
            uint8_t class_occupancy: 1;
            uint8_t class_bridge: 1; // hub
          };
        uint8_t class_id;
      };
    union {
        struct
          {
            uint8_t class_gateway: 1;
            uint8_t class_reserved: 7;
          };
        uint8_t classx_id;
      };
    uint8_t dcc_vendor;
...
typedef struct PACKED           // t_bidib_cs_accessory
  {
    union
      {
        struct
...
typedef struct PACKED  // t_bidib_bin_state
  {
    union
      {
        struct
...
typedef struct PACKED  // t_bidib_cs_drive
  {
    union
      {
        struct
...
typedef struct PACKED  // t_bidib_cs_pom
  {
    union
      {
        struct
...
typedef struct PACKED // t_bidib_cs_prog
  {
...
typedef struct PACKED  // t_bidib_cs_prog_state
  {
    uint8_t result;                   //
    uint8_t time;                     //
    union
...
typedef struct PACKED // t_bidib_msg_type_filter
  {
    uint8_t msg_type_system: 1; // MSG_xSYS
    uint8_t msg_type_config: 1; // MSG_xFC
    uint8_t msg_type_occupancy: 1; // MSG_xBM
    uint8_t msg_type_booster: 1; // MSG_xBST
    uint8_t msg_type_switch: 1; // MSG_xLC, MSG_xMAC
    uint8_t msg_type_guest: 1; // MSG_xDIS
    uint8_t msg_type_dcc: 1; // MSG_xGEN
    uint8_t msg_type_accessory: 1; // MSG_xACC
  } t_bidib_msg_type_filter;

typedef struct PACKED // t_bidib_guest_subscription
  {
    t_bidib_msg_type_filter downstream;
    t_bidib_msg_type_filter upstream;
  } t_bidib_guest_subscription;
...
#define FEATURE_BM_RCPLUS_AVAILABLE        29   // 1: RailcomPlus messages available (deprecated)
#define FEATURE_BM_EXT_AVAILABLE           29   // Bit 0: RCPLUS available; Bit 2: DCCA available.
//-- occupancy
...
#define FEATURE_GEN_EXT_ENABLED            114  // bitfield enabled messages/modes: 0 - RCPLUS, 1 - M4, 2 - DCCA, other: reserved

//-- system
#define FEATURE_STRING_NAMESPACES_AVAILABLE 250 // bitfield, bit index corresponds to namespace
...
//===============================================================================
//
// 6. Defines for transport layers: Serial Link, BiDiBus, netBiDiB
//
//===============================================================================
...
#define BIDIB_LINK_DESCRIPTOR_ROLE               0x7F // 2:role (1=interface, 2=node, 3=both)
...
// 7.a) System messages
// system identifiers (in MSG_SYS_MAGIC)
#define BIDIB_SYS_MAGIC         0xAFFE                  // full featured BiDiB-Node
#define BIDIB_BOOT_MAGIC        0xB00D                  // reduced Node, bootloader only

// namespaces of strings
#define BIDIB_STRING_NAMESPACE_NODE_INFO      0x00  // general information about the node as a whole
#define BIDIB_STRING_NAMESPACE_DEBUG_STREAM   0x01  // text terminal and debug logging
#define BIDIB_STRING_NAMESPACE_TEXT_DISPLAY   0x02  // texts to output on displays via accessory commands
#define BIDIB_STRING_NAMESPACE_RW_STORAGE     0x03  // read/write text to storage on node
...
// 9.a) Accessory
// Accessory parameter
#define BIDIB_ACCESSORY_PARA_USES_STRINGS  249   // following boolean declares whether aspect refers to MSG_STRING texts
// #define BIDIB_ACCESSORY_PARA_HAS_INFO   249   // (deprecated?) following boolean declares whether ACC state will supply additional infos
...
// 9.b) Port control and configuration
// type codes
...
// Operation control codes (for output ports and macros) - limited to one nibble
...
//===============================================================================
//
// 10. Defines for Distributed Control
//
//===============================================================================

// target modes for address resolution
#define BIDIB_TARGET_MODE_UID            0x00 // dispatch by uid, followed by 5 bytes (vid, pid[4])
#define BIDIB_TARGET_MODE_ALL            0x01 // global subscription
#define BIDIB_TARGET_MODE_TOP            0x02 // dispatch to root node in the bidib system tree
// subscriptions by class in the same order as bits in t_bidib_unique_id.class_id
#define BIDIB_TARGET_MODE_SWITCH         0x08 // dispatch to nodes with peripherals
#define BIDIB_TARGET_MODE_BOOSTER        0x09 // dispatch to booster nodes
#define BIDIB_TARGET_MODE_ACCESSORY      0x0A // dispatch to nodes with accessories
// #define BIDIB_TARGET_MODE_DCCPROG     0x0B // dispatch to track signal generator nodes with service mode
#define BIDIB_TARGET_MODE_DCCGEN         0x0C // dispatch to track signal generator nodes
// #define BIDIB_TARGET_MODE_HOST        0x0D // dispatch to a guest node representing the host
// #define BIDIB_TARGET_MODE_OCCUPANCY   0x0E // dispatch to nodes with occupancy sensors
#define BIDIB_TARGET_MODE_HUB            0x0F // dispatch to nodes with subnodes / hub functionalities

//===============================================================================
//
// 11. Defines for RailcomPlus
//
//===============================================================================
...
// 10.a) Opcodes for MSG_CS_RCPLUS
...
// 10.b) Opcodes for MSG_CS_RCPLUS_ACK
...
// 10.c) Opcodes for MSG_BM_RCPLUS
...
// 11.b) Opcodes for MSG_CS_M4_ACK
...
// 11.c) Opcodes for MSG_CS_POM/MSG_BM_XPOM
...
// DCCA Namesspaces (if not yet defined - see dcc_advanced.h)
    ...
    // decoder name spaces
    #define DCCA_SPACE_SHORTINFO           -1       // fix size, not in name spaces
    #define DCCA_SPACE_EXTENDED_CAPA        0       // variable size
    #define DCCA_SPACE_SPACEINFO            1       // variable size
    #define DCCA_SPACE_SHORTGUI             2       // variable size
    #define DCCA_SPACE_CV                   3       // variable size
    #define DCCA_SPACE_FUNCICON             4       // variable size
    #define DCCA_SPACE_LONGNAME             5       // variable size
    #define DCCA_SPACE_PRODINFO             6       // variable size
    #define DCCA_SPACE_LOCOINFO             7       // variable size
#endif

// 12.a) Opcodes for MSG_CS_DCCA (downstream)
...
#define BIDIB_DCCA_LOGON_ASSIGN           0x08 // 2..5:dec_mun, 6,7:dec_mid, 8:new_addrl, 9:new_addrh
#define BIDIB_DCCA_GET_TID                0x0D // -
#define BIDIB_DCCA_SET_TID                0x0E // 2:cid[0],3:cid[1], 4:sid

#define BIDIB_DCCA_GET_DATA               0x0F // 2:count (old: BIDIB_DCCA_GET_INFO_CONT)

#define BIDIB_DCCA_SELECT                 0x10 // range 0x10 ... 0x1F: 2..5:addr/did, 6,7:mid, 8:namespace, 9..12:cv
#define BIDIB_DCCA_SELECT_SHORTINFO       0x1F
#define BIDIB_DCCA_SELECT_READ_BLOCK      0x1E
#define BIDIB_DCCA_SELECT_READ_BACKGROUND 0x1D
#define BIDIB_DCCA_SELECT_WRITE_BLOCK     0x1C
#define BIDIB_DCCA_SELECT_SET_DEC_STATE   0x1B

// 12.b) Opcodes for MSG_CS_DCCA_ACK
...
// 12.c) Opcodes for MSG_BM_DCCA

#define BIDIB_DCCA_LOGON_COLLISION        0x80 // -
#define BIDIB_DCCA_LOGON_NEW_DID          0x81 // 3:dec_mun[0],4:dec_mun[1],5:dec_mun[2],6:dec_mun[3], 7:dec_mid[0] 8:dec_mid[1]
#define BIDIB_DCCA_DATA_SHORTINFO         0x82 // 3-8: decuid, 9..13:shortinfo
#define BIDIB_DCCA_LOGON_ASSIGN_ACK       0x83 // 3-8: decuid; 9..13:decstate (response to addr assignment)

#define BIDIB_DCCA_INFO                   0x90 // 1..4:addr/did, 5:0/vid, [6..n:parameter]

// answer to select
#define BIDIB_DCCA_SELECT_ACK_RB          0x8F // 3:dec_mun[0],4:dec_mun[1],5:dec_mun[2],6:dec_mun[3], 7:dec_mid[0] 8:dec_mid[1]; Decoder sends 8 ACK

// answer to data queries
#define BIDIB_DCCA_DATA                   0x90 // 3-8:decuid, 9:namespace, 10,11:stream index, 12..42:stream data
